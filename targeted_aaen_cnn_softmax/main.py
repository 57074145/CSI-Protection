...
protect_type_num = 6 # The total number of categories
...
rerank_holder = tf.placeholder(tf.float32, [None, protect_type_num])
protection_target = 5 # targeted label
...
def get_test_batch(faces,model_1_label,model_2_label, model_3_label, batch_size):
    batch_xs = faces
    model_1_batch_ys = model_1_label
    model_2_batch_ys = model_2_label
    model_3_batch_ys = model_3_label
    
    return batch_xs, model_1_batch_ys, model_2_batch_ys, model_3_batch_ys 
...
def test():
    ...
    batch_xs, target_1_batch_ys, target_2_batch_ys, target_3_batch_ys = get_test_batch(temp_test_x, target_1_test_y, target_2_test_y, target_3_test_y, batch_size) #generate input CSI 
    ...
    with tf.Session() as sess:
        ...
        adv_images = sess.run(
            ...
            feed_dict={images_holder: batch_xs} # adversarial signal generation 
        )   
        ...
        acc_a = np.mean(correct_prediction)
       
    
        print('Protected success rate:{0:0.5f}'.format(acc_a))
        ...
        show_ori = reverse_norm(batch_xs,  train_x_max, train_x_min)
        show_adv = reverse_norm(adv_images, train_x_max, train_x_min)


        sio.savemat('saveddata.mat', {'show_ori': show_ori,'show_adv': show_adv, 'ori_ft_1': ori_ft_1.reshape(-1, 6, 64), 'adv_ft_1': adv_ft_1.reshape(-1, 6, 64), 
