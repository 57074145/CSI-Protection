...
train_y = np.transpose(data['train_act'][:])
test_y = np.transpose(data['test_act'][:])
...
readout_weight = weight_variable("W_fc2", [128, num_classes])
readout_bias = bias_variable("b_fc2", [num_classes])
...
y_conv = tf.matmul(h_fc1_drop, readout_weight) + readout_bias # SVM classifiers

regularization_loss = tf.reduce_mean(tf.square(readout_weight))

hinge_loss = tf.reduce_mean(tf.square(tf.maximum(tf.zeros([batch_size, num_classes]), 1 - y_ * y_conv)))

loss = regularization_loss + penalty_parameter * hinge_loss # SVM loss

optimizer = tf.train.AdamOptimizer(learning_rate=1e-3).minimize(loss)

output = tf.identity(tf.sign(y_conv), name='prediction')
...
for i in range(0,max_iter):
    ...
    if (i+1) % 100 == 0: 
        saver.save(sess, "movement/cnn_net.ckpt", i)     
...
test_accuracy, prediction = sess.run([accuracy, output], feed_dict=feed_dict) 
