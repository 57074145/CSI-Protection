...
class SecondBasicCnn:
    ...
    def prediction(self):
        ...
        y_conv = tf.nn.softmax(tf.matmul(h_fc1_drop, self.weights['W_fc2']) + self.biases['b_fc2'])
    ...
    def loss(self):
        ...
        cross_entropy = tf.reduce_mean(-tf.reduce_sum(self.groundtruth * tf.log(tf.clip_by_value(pre,1e-10,tf.reduce_max(pre))), reduction_indices=[1])) # cross_entropy

        return cross_entropy # softmax loss
